...
namespace NzbDrone.Core.Applications.Whisparr
{
    public class Whisparr : ApplicationBase<WhisparrSettings>
    {
        ...
        private WhisparrIndexer BuildWhisparrIndexer(IndexerDefinition indexer, DownloadProtocol protocol, int id = 0)
        {
            ...
            var syncFields = new[] { "baseUrl", "apiPath", "apiKey", "categories", "additionalParameters", "minimumSeeders", "seedCriteria.seedRatio", "seedCriteria.seedTime" };
            ...
            whisparrIndexer.Fields.FirstOrDefault(x => x.Name == "additionalParameters").Value = $"&prowlarr-app-source={HttpUtility.UrlEncode(Definition.Name)}";
