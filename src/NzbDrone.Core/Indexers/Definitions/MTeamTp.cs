...
public class MTeamTpRequestGenerator : IIndexerRequestGenerator
{
    ...
    public IndexerPageableRequestChain GetSearchRequests(MovieSearchCriteria searchCriteria)
    {
        ...
        pageableRequests.Add(GetPagedRequests(MTeamTpRequestType.General, searchCriteria, searchCriteria.SanitizedSearchTerm, searchCriteria.FullImdbId));
    ...
    public IndexerPageableRequestChain GetSearchRequests(TvSearchCriteria searchCriteria)
    {
        ...
        pageableRequests.Add(GetPagedRequests(MTeamTpRequestType.General, searchCriteria, searchCriteria.SanitizedTvSearchString, searchCriteria.FullImdbId));
    ...
    public IndexerPageableRequestChain GetSearchRequests(MusicSearchCriteria searchCriteria)
    {
        ...
        pageableRequests.Add(GetPagedRequests(MTeamTpRequestType.Music, searchCriteria, searchCriteria.SanitizedSearchTerm));
    ...
    public IndexerPageableRequestChain GetSearchRequests(BookSearchCriteria searchCriteria)
    {
        ...
        pageableRequests.Add(GetPagedRequests(MTeamTpRequestType.General, searchCriteria, searchCriteria.SanitizedSearchTerm));
    ...
    private IEnumerable<IndexerRequest> GetPagedRequests(MTeamTpRequestType requestType, SearchCriteriaBase searchCriteria, string searchTerm, string imdbId = null)
    {
        ...
        switch (requestType)
        {
            case MTeamTpRequestType.Adult when _settings.AdultContent && adultCategories.Any():
                yield return BuildSearchRequest(SearchAdultUrl, adultCategories, searchTerm, page: page);
                break;
            case MTeamTpRequestType.Music when musicCategories.Any():
                yield return BuildSearchRequest(SearchMusicUrl, musicCategories, searchTerm, page: page);
                break;
            case MTeamTpRequestType.General when !categoryMapping.Any() || generalCategories.Any():
    ...
    private IndexerRequest BuildSearchRequest(string searchUrl, List<string> categories = null, string searchTerm = null, string imdbId = null, int page = 0)
    {
        ...
        {
            categories.ForEach(cat => requestBuilder = requestBuilder.AddQueryParam($"cat{cat}", "1"));
        }
    ...
    public IList<ReleaseInfo> ParseResponse(IndexerResponse indexerResponse)
    {
        if (indexerResponse.HttpResponse.StatusCode != HttpStatusCode.OK)
        {
            ...
            if (indexerResponse.HttpResponse.HasHttpRedirect)
            {
                ...
                {
                    throw new IndexerAuthException("Redirected to the login page. Most likely your session expired or was killed. Recheck your cookie and try testing the indexer.");
                }

                if (indexerResponse.HttpResponse.RedirectUrl.ContainsIgnoreCase("verify.php"))
                {
                    throw new IndexerAuthException("Redirected to the 2FA verify page. Recheck your cookie and try testing the indexer.");
                }

                throw new IndexerAuthException($"Redirected to {indexerResponse.HttpResponse.RedirectUrl} from API request");
            }
